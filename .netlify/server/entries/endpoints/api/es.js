...
var import_store = require("../stores/store.js");
...
async function post({ request }, event) {
  ...
  try {
    let response = await import_axios.default.post("http://localhost:3000/api/commands", {
      "aggregateName": "Users",
      "type": "create",
      "aggregateId": body.id,
...
async function get({ url: { searchParams } }) {
  ...
  let response = await import_axios.default.get(`http://localhost:3000/api/query/userStats/getUserByPasswordAndEmail?email=${searchParams.get("email")}&password=${searchParams.get("password")}`);
